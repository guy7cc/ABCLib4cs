...
namespace LibraryMerger.Core.Rewriter
{
    internal class FullyQualifyABCTypesRewriter : CSharpSyntaxRewriter
    {
        ...
        public override SyntaxNode VisitIdentifierName(IdentifierNameSyntax node)
        {
            ...
            if (symbolInfo.Symbol != null && IsFromABCLib(symbolInfo.Symbol))
            {
        ...
        {
            if (!symbol.IsStatic && symbol is not ITypeSymbol) return false;
            if (symbol is ITypeParameterSymbol) return false;
            return symbol.ContainingNamespace?.ToDisplayString().StartsWith("ABCLib4cs") ?? false;
        }
    }
}